# u-net矢量化V2.5.py
# V2.5: V8模型架构 + V3推理增强 (percentile归一化 + 阈值信息读取)
# 修复大图推理时漏检小尺寸近似正方形地块的问题
# 基于 V2 的模型定义 + V3 的推理管线改进
...
class MultiTaskUNet(nn.Module):
    """V8 版本的多任务 UNet（ConvTranspose2d 头 + 原始浅融合）"""
    def __init__(self, encoder_name="resnet34", encoder_weights="imagenet", in_channels=4, classes=1,
                 use_cbam=True, use_spectral_attention=True, dropout_rate=0.2):
        super().__init__()
...
# ============================================================================
# V2.5 推理引擎: V8模型 + V3推理增强
# ============================================================================
class InferenceEngine:
    """V2.5: V2 经典引擎 + percentile 归一化选项，保证 legacy 模式行为与 V2 完全一致"""
    ...
    def _read_image(self, path, in_channels=None):
        """读图，返回未归一化的原始 float32（与 V2 完全一致）"""
...
def run_inference_pipeline(
    ...
    seg_threshold=0.5, boundary_threshold=0.15,
    ...
    print(f"\n{'=' * 60}")
    print(f"推理完成! 总计 {len(all_polygons)} 个地块")
    if save_shp:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="U-Net V2.5 推理矢量化 (V8/V8.5模型 + V3推理增强)")
    ...
    parser.add_argument("--output", type=str, default="./predictions_v25", help="输出目录")
    ...
    parser.add_argument("--norm_mode", type=str, default="percentile", choices=["percentile", "legacy"],
                        help="归一化模式 (percentile=V8.5默认, legacy=V8兼容)")
