...
# RTX 4090 优化版：更大 batch、混合精度、多 worker 加速、针对 10000 张数据调参
import os
...
class FarmlandDataset(Dataset):
    ...
    def _read_mask(self, img_path):
        ...
        if not mask_path.exists():
            mask_path_tif = self.mask_dir / f"{img_path.stem}.tif"
            mask_path = mask_path_tif if mask_path_tif.exists() else mask_path
...
class LogCoshDiceLoss(nn.Module):
    """Log-Cosh Dice Loss: smoother optimization, better boundary completeness (PRUE 2025)."""
...
class TverskyLoss(nn.Module):
    """Tversky Loss: for imbalanced field-boundary segmentation (PRUE 2025)."""
...
class CombinedLoss(nn.Module):
    def __init__(self, loss_type="log_cosh_dice", dice_weight=0.5, aux_weight=0.5, tversky_alpha=0.3, tversky_beta=0.7):
        super().__init__()
    ...
    def forward(self, inputs, targets):
        primary_loss = self.primary(inputs, targets)
        aux_loss = self.aux(inputs, targets)
        return self.dice_weight * primary_loss + self.aux_weight * aux_loss
...
class Trainer:
    def __init__(self, config):
        ...
        print(">>> 使用多任务模型 (Multi Task + CBAM + Spectral Attention + Distance) - V8")
        ...
        self.train_loader = DataLoader(train_ds, batch_size=config["batch_size"], shuffle=True, num_workers=num_workers, pin_memory=True, drop_last=True)
        self.val_loader = DataLoader(val_ds, batch_size=config["batch_size"], shuffle=False, num_workers=num_workers, pin_memory=True)
    ...
    def _load_checkpoint(self, ckpt_path):
        ...
        if not self.config.get("reset_optimizer", False):
            ...
            try:
                ...
                print(f"  调度器状态不匹配，已重新初始化 ({e})")
        self.start_epoch = ckpt.get("epoch", 0) if not self.config.get("reset_optimizer", False) else 0
        self.best_iou = ckpt.get("best_iou", 0.0)
        self.best_bdy_iou = ckpt.get("best_bdy_iou", 0.0)
        self.train_losses = [] if self.config.get("reset_optimizer", False) else ckpt.get("train_losses", [])
        self.val_ious = [] if self.config.get("reset_optimizer", False) else ckpt.get("val_ious", [])
        self.val_bdy_ious = [] if self.config.get("reset_optimizer", False) else ckpt.get("val_bdy_ious", [])
        self.boundary_losses = [] if self.config.get("reset_optimizer", False) else ckpt.get("boundary_losses", [])
        self.distance_losses = [] if self.config.get("reset_optimizer", False) else ckpt.get("distance_losses", [])
        tag = "微调 (仅加载权重)" if self.config.get("reset_optimizer", False) else "恢复训练"
        print(f">>> 从 {ckpt_path} {tag} (best_iou={self.best_iou:.4f})")
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch + 1, self.config["epochs"] + 1):
            ...
            if epoch <= self.warmup_epochs:
                ...
                if self.config.get("scheduler_type", "plateau") == "cosine":
            ...
            if val_iou > self.best_iou:
                ...
                if patience_counter >= early_stopping_patience:
                    print(f"\n\U0001f6d1 早停触发！连续{early_stopping_patience}轮未提升")
                    break
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        if props.total_memory >= 20 * 1024**3:
            default_bs, default_lr = 32, 5e-4
        elif props.total_memory >= 10 * 1024**3:
            default_bs, default_lr = 16, 3e-4
        else:
            default_bs, default_lr = 8, 2e-4
    else:
        default_bs, default_lr = 4, 1e-4

    parser = argparse.ArgumentParser(description="U-Net Training V8_4090 (Optimized for RTX 4090)")
    ...
    parser.add_argument("--epochs", type=int, default=100, help="Epochs (10000图推荐80-100)")
    parser.add_argument("--lr", type=float, default=default_lr, help="Learning rate")
    parser.add_argument("--num_workers", type=int, default=8, help="Num workers (4090推荐8-12)")
