# u-net--baseline-DeepLabV3+.py
# 论文基线: DeepLab v3+ (ResNet50编码器 + ASPP + 低层特征解码器)
# RTX 4090 优化版 (batch=32, workers=8, epoch=150)
# 与 V8 使用相同的训练流程、数据加载、评估指标
...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        size = x.shape[2:]
        c1 = self.conv1x1(x)
        c2 = self.conv3x3_d6(x)
        c3 = self.conv3x3_d12(x)
        c4 = self.conv3x3_d18(x)
        c5 = F.interpolate(self.image_pool(x), size=size, mode="bilinear", align_corners=False)
        return self.fuse(torch.cat([c1, c2, c3, c4, c5], dim=1))


class DeepLabV3Plus(nn.Module):
    """
    DeepLab v3+ 手动实现
    编码器: ResNet50 + ASPP + 低层特征融合解码器
    """
    def __init__(self, in_channels=4, num_classes=1, encoder_name="resnet50"):
        ...
        self.layer1 = base.layer1   # 1/4,  256ch
        self.layer2 = base.layer2   # 1/8,  512ch
        self.layer3 = base.layer3   # 1/16, 1024ch
        self.layer4 = base.layer4   # 1/32, 2048ch → ASPP
        ...
        self.low_conv = nn.Sequential(
            nn.Conv2d(enc_ch[1], 48, 1, bias=False),
    ...
    def forward(self, x):
        ...
        aspp_out = self.aspp(f4)                          # 1/32
        aspp_up = F.interpolate(aspp_out, size=f1.shape[2:], mode="bilinear", align_corners=False)
        low = self.low_conv(f1)
        dec = self.decoder(torch.cat([aspp_up, low], dim=1))
...
class Trainer:
    def __init__(self, config):
        ...
        print(f"\n{'='*60}")
        print(f"DeepLab v3+ ({config['encoder_name']})")
        print(f"{'='*60}")
    ...
    def plot_curves(self):
        ...
        plt.suptitle(f"DeepLab v3+ ({self.config['encoder_name']})", fontsize=14)
        plt.tight_layout()
    ...
    def run(self):
        ...
        self.plot_curves()
        print(f"\n{'='*60}")
        print(f"训练完成! DeepLab v3+ ({self.config['encoder_name']})")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="DeepLab v3+ 基线模型训练")
    parser.add_argument("--data_root", required=True)
    parser.add_argument("--output_dir", required=True)
    parser.add_argument("--encoder_name", default="resnet50", choices=["resnet50", "resnet101"])
    ...
    config["resume_path"] = config.pop("resume", None)  # --resume → resume_path
    Trainer(config).run()
