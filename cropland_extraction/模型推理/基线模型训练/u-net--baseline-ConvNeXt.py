# u-net--baseline-ConvNeXt.py
# 论文基线: ConvNeXt-Tiny (CVPR 2022) + FPN 解码器
# RTX 4090 优化版
# 需服务器环境: pip install timm huggingface_hub
...
# ============================================================================
# 模型: ConvNeXt-Tiny + FPN 解码器
# ============================================================================

class ConvNeXtFPN(nn.Module):
    """
    ConvNeXt-Tiny (CVPR 2022) + 轻量 FPN 解码器
    骨干: timm convnext_tiny, pretrained=True (ImageNet-1K)
    4阶段输出: [96, 192, 384, 768] @ 1/4, 1/8, 1/16, 1/32
    """
    def __init__(self, in_channels=4, num_classes=1, backbone_name="convnext_tiny"):
        super().__init__()
        import timm

        # 创建原始 ConvNeXt 并加载预训练权重（不使用 features_only 包装器）
        self.backbone = timm.create_model(backbone_name, pretrained=True, features_only=False)

        # 适配 4 通道 stem
        old_c = self.backbone.stem[0]
        new_c = nn.Conv2d(
            in_channels, old_c.out_channels,
            kernel_size=old_c.kernel_size, stride=old_c.stride,
            padding=old_c.padding, bias=old_c.bias is not None,
        )
        with torch.no_grad():
            new_c.weight[:, :3] = old_c.weight
            if in_channels > 3:
                m = old_c.weight.mean(dim=1, keepdim=True)
                for ch in range(3, in_channels):
                    new_c.weight[:, ch:ch+1] = m
        self.backbone.stem[0] = new_c
        ...
        self.lateral_convs = nn.ModuleList([
            nn.Conv2d(ch, 128, kernel_size=1) for ch in feat_channels
        ])
        self.fpn_conv = nn.Sequential(
            nn.Conv2d(128 * len(feat_channels), 128, 3, padding=1, bias=False),
    ...
    def forward(self, x):
        ins = x.shape[2:]
        # 手动提取多尺度特征（兼容所有 timm 版本，无需 features_only 包装）
        x = self.backbone.stem(x)              # 1/4,   96ch
        f1 = x                                 # 1/4
        x = self.backbone.stages[0](x)         # 1/4,   96ch (跳过，同上尺寸)
        x = self.backbone.stages[1](x)         # 1/8,  192ch
        f2 = x                                 # 1/8
        x = self.backbone.stages[2](x)         # 1/16, 384ch
        f3 = x                                 # 1/16
        x = self.backbone.stages[3](x)         # 1/32, 768ch
        f4 = x                                 # 1/32
        feats = [f1, f2, f3, f4]
        tsize = feats[0].shape[2:]
        fused = torch.cat([
            F.interpolate(conv(f), size=tsize, mode="bilinear", align_corners=False)
            for f, conv in zip(feats, self.lateral_convs)
        ], dim=1)
        out = self.fpn_conv(fused)
        out = F.interpolate(out, size=ins, mode="bilinear", align_corners=False)
...
class Trainer:
        ...
        for batch_idx, (imgs, masks) in enumerate(pbar):
            ...
            if self.config["use_amp"]:
                with autocast():
                    ...
                    loss = self.criterion(logits, masks) / gas
    ...
    def plot_curves(self):
        ...
        plt.suptitle("ConvNeXt-Tiny + FPN", fontsize=14)
        plt.tight_layout()
    ...
    def run(self):
        ...
        self.plot_curves()
        print(f"\n{'='*60}")
        print(f"训练完成! ConvNeXt-Tiny + FPN")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="ConvNeXt-Tiny 基线模型训练")
