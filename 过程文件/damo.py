...
if __name__ == '__main__':
    ...
    model_path = train_pipeline(
        model_type='rf',  # 选择随机森林模型rf (Random Forest) - 随机森林，svm (Support Vector Machine) - 支持向量机，xgboost (XGBoost) - 梯度提升树，lgbm (LightGBM) - 轻量级梯度提升机，深度学习模型-cnn (Convolutional Neural Network) - 卷积神经网络
        shp_path=r'/\data\完整作物边界\作物分布.shp',  # 训练数据
        tif_path=r'/\data\Sentinel2_TimeSeries_Stack_2023-0000000000-0000000000.tif',  # 遥感影像
...
# 步骤2: 模型推理
    ...
    output_shp = predict_pipeline(
        model_path=model_path,  # 训练好的模型，可以填本地模型路径
        tif_path=r'/\data\Sentinel2_TimeSeries_Stack_2023-0000000000-0000006400.tif',  # 新影像
        shp_path=r'/\data\完整作物边界\作物分布.shp',  # 待预测地块
